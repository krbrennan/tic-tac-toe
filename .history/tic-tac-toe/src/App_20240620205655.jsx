...
function App() {
  ...
  function Square({id, value, onSquareClick}) {
    ...
    if(isThisAWinningSquare) {
      return(
        <button id={id} onClick={onSquareClick} className='cell green'>{value}</button>
      )
    } else {
      return(
    ...
    return(
      ...
      <div className="reset-btn-container">
        ...
        <div className="row r-1">
          <Square winning={ winningCombo.includes(0) ? true : false } onSquareClick={()=> {handleClick(0)}} value={squares[0]} />
          <Square winning={ winningCombo.includes(1) ? true : false } onSquareClick={()=> {handleClick(1)}} value={squares[1]}/>
          <Square winning={ winningCombo.includes(2) ? true : false } onSquareClick={()=> {handleClick(2)}} value={squares[2]}/>
        </div>
        <div className="row r-2">
          <Square winning={ winningCombo.includes(3) ? true : false } onSquareClick={()=> {handleClick(3)}} value={squares[3]} />
          <Square winning={ winningCombo.includes(4) ? true : false } onSquareClick={()=> {handleClick(4)}} value={squares[4]} />
          <Square winning={ winningCombo.includes(5) ? true : false } onSquareClick={()=> {handleClick(5)}} value={squares[5]} />
        </div>
        <div className="row r-3">
          <Square winning={ winningCombo.includes(6) ? true : false } onSquareClick={()=> {handleClick(6)}} value={squares[6]} />
          <Square winning={ winningCombo.includes(7) ? true : false } onSquareClick={()=> {handleClick(7)}} value={squares[7]} />
          <Square winning={ winningCombo.includes(8) ? true : false } onSquareClick={()=> {handleClick(8)}} value={squares[8]} />
        </div>
    </>
    )
  }
